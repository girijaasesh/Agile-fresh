...
export default function QuickRegisterClient() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      confirmUrl.searchParams.set('ref',    String(data.id).slice(0, 8).toUpperCase());
          ...
          <div className="qr-grid" style={{ alignItems: 'center' }}>
            ...
            <div ref={formRef}>
              <div className="qr-form-card">
                {phase === 'form'    && <RegistrationForm form={form} set={set} blur={blur} fieldErr={fieldErr} cert={cert} sessions={sessions} session={session} price={price} eb={eb} seats={seats} urgency={urgency} submitting={submitting} apiError={apiError} onSubmit={handleSubmit} authSession={authSession} authStatus={status} onGoogleSignIn={() => signIn('google', { callbackUrl: window.location.href })} onSignOut={() => signOut({ callbackUrl: window.location.href })} />}
                {phase === 'payment' && <PaymentSection   form={form} cert={cert} session={session} price={price} regId={regId} onSuccess={() => { window.location.href = '/registration-success'; }} />}
              </div>
            </div>
          </div>
        </div>
...
function RegistrationForm({ form, set, blur, fieldErr, cert, sessions, session, price, eb, seats, urgency, submitting, apiError, onSubmit, authSession, authStatus, onGoogleSignIn, onSignOut }) {
  return (
    <form onSubmit={onSubmit} noValidate>
      ...
      <div style={{ marginBottom: 16, textAlign: 'center' }}>
        ...
        {authSession ? (
          <div style={{ fontSize: 13, color: '#0B1629' }}>
          ...
          <button type="button" onClick={onGoogleSignIn} className="btn btn-outline btn-full" style={{ marginBottom: 12 }}>
