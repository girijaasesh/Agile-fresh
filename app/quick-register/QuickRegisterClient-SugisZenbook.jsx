...
export default function QuickRegisterClient() {
  ...
  const handleGoogleSuccess = useCallback((credentialResponse) => {
    ...
    setGoogleUser({ name: name || '', email: email || '', picture: picture || '' });
    setForm(f => ({
      ...f,
      name:  name  || f.name,
      email: email || f.email,
  ...
  const handleGoogleError = useCallback(() => {
    console.warn('Google sign-in failed or was dismissed');
  }, []);
          ...
          <div className="qr-grid" style={{ alignItems: 'center' }}>
            ...
            <div ref={formRef}>
              <div className="qr-form-card">
                {phase === 'form'    && <RegistrationForm form={form} set={set} blur={blur} fieldErr={fieldErr} cert={cert} sessions={sessions} session={session} price={price} eb={eb} seats={seats} urgency={urgency} submitting={submitting} apiError={apiError} onSubmit={handleSubmit} googleUser={googleUser} onGoogleSuccess={handleGoogleSuccess} onGoogleError={handleGoogleError} />}
        ...
        <div className="qr-wrap">
          ...
          <div className="qr-pricing-grid">
            {[
              ...
              const c   = getCert(id);
              const s   = getSessions(id)[0];
              const isEB = s && isEarlyBird(s.ebDeadline);
              return (
          ...
          <p style={{ marginTop: 20, fontSize: 13, color: 'var(--slate)' }}>
            Looking for RTE, SPC, or Advanced Scrum Master?{' '}
            <button onClick={scrollToForm} style={{ background: 'none', border: 'none', color: 'var(--gold)', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>View all courses in the form →</button>
          </p>
        </div>
      </section>
...
function RegistrationForm({ form, set, blur, fieldErr, cert, sessions, session, price, eb, seats, urgency, submitting, apiError, onSubmit, googleUser, onGoogleSuccess, onGoogleError }) {
  return (
    <form onSubmit={onSubmit} noValidate>
      ...
      {googleUser ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 10, padding: '10px 14px', marginBottom: 20 }}>
          {googleUser.picture && <img src={googleUser.picture} alt="" style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0 }} referrerPolicy="no-referrer" />}
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#15803D' }}>✓ Signed in with Google</div>
            <div style={{ fontSize: 12, color: '#64748B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{googleUser.name} · {googleUser.email}</div>
          </div>
        </div>
      ) : (
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 12, color: '#94A3B8', whiteSpace: 'nowrap' }}>Sign in to pre-fill your details</span>
            <div style={{ flex: 1, height: 1, background: '#E2E8F0' }} />
          </div>
      ...
      <Field label="Select Course *" error={fieldErr('certId')}>
        <select className={`qr-input${fieldErr('certId') ? ' err' : ''}`} value={form.certId}
          ...
          {CERTIFICATIONS.map(c => (
            <option key={c.id} value={c.id}>{c.code} · {c.title} — ${c.earlyBird.toLocaleString('en-US')} (Early Bird)</option>
          ))}
        </select>
      </Field>
